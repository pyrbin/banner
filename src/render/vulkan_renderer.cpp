...
{
    auto app_info = vk::ApplicationInfo("TD Engine", VK_MAKE_VERSION(1, 0, 0), "No engine",
    ...
    _instance = vk::createInstanceUnique(
        { vk::InstanceCreateFlags(), &app_info, u32(_instance_layers.size()),
          _instance_layers.data(), u32(_instance_extensions.size()), _instance_extensions.data() });
...
void vulkan_renderer::create_logical_device()
{
    ...
    _device = static_cast<vk::UniqueDevice>(_physical_device.createDeviceUnique(
        { vk::DeviceCreateFlags(), u32(queue_infos.size()), queue_infos.data(),
          u32(_instance_layers.size()), _instance_layers.data(), u32(_device_extensions.size()),
          _device_extensions.data(), &device_features }));
...
void vulkan_renderer::create_swapchain()
{
    ...
    _swapchain = _device->createSwapchainKHR(
        { vk::SwapchainCreateFlagsKHR(), _surface.get(), image_count, _swap_surface_format.format,
          _swap_surface_format.colorSpace, _swap_extent, 1,
          vk::ImageUsageFlagBits::eColorAttachment, _image_sharing_mode,
          u32(_queue_family_indices.size()), _queue_family_indices.data(),
          vk::SurfaceTransformFlagBitsKHR::eIdentity, vk::CompositeAlphaFlagBitsKHR::eOpaque,
          _swap_present_mode, VK_TRUE, _old_swapchain });
...
void vulkan_renderer::create_swapchain_images()
{
    ...
    for (const auto& img : _swapchain_images) {
        _swapchain_image_views.push_back(_device->createImageView({
            vk::ImageViewCreateFlags(), img, vk::ImageViewType::e2D, _swap_surface_format.format,
...
void vulkan_renderer::create_depth_stencil_image()
{
    ...
    _depth_image = _device->createImageUnique(
        { vk::ImageCreateFlags(), vk::ImageType::e2D, _depth_buffer_format,
          vk::Extent3D(_swap_extent, 1), 1, 1, vk::SampleCountFlagBits::e1,
          vk::ImageTiling::eOptimal, vk::ImageUsageFlagBits::eDepthStencilAttachment,
          _image_sharing_mode, u32(_queue_family_indices.size()), _queue_family_indices.data(),
    ...
    _depth_image_view = _device->createImageViewUnique(
        { vk::ImageViewCreateFlags(), _depth_image.get(), vk::ImageViewType::e2D,
          _depth_buffer_format, vk::ComponentMapping(),
...
void vulkan_renderer::create_render_pass()
{
    ...
    vector<vk::AttachmentDescription> attachments{
        ...
        { vk::AttachmentDescriptionFlags(), _depth_buffer_format, vk::SampleCountFlagBits::e1,
    ...
    vector<vk::SubpassDescription> subpasses = {
        vk::SubpassDescription{ vk::SubpassDescriptionFlags(), vk::PipelineBindPoint::eGraphics, 0,
                                nullptr, u32(stencil_reference.size()), stencil_reference.data(),
    ...
    vector<vk::SubpassDependency> dependency{
        ...
        { VK_SUBPASS_EXTERNAL, 0, vk::PipelineStageFlagBits::eColorAttachmentOutput,
          vk::PipelineStageFlagBits::eColorAttachmentOutput, vk::AccessFlagBits(0),
    ...
    _renderpass = _device->createRenderPass(
        { vk::RenderPassCreateFlags(), u32(attachments.size()), attachments.data(),
          u32(subpasses.size()), subpasses.data(), u32(dependency.size()), dependency.data() });
}

void vulkan_renderer::create_shader()
{
    auto vert_shader_module = load_shader_module("vert.spv");
    auto frag_shader_module = load_shader_module("frag.spv");

    // Vertex shader stage
    const vk::PipelineShaderStageCreateInfo vert_info{ vk::PipelineShaderStageCreateFlags(),
                                                       vk::ShaderStageFlagBits::eVertex,
                                                       vert_shader_module, "main" };

    // Fragment shader stage
    const vk::PipelineShaderStageCreateInfo frag_info{ vk::PipelineShaderStageCreateFlags(),
...
void vulkan_renderer::create_graphics_pipeline()
{
    ...
    auto viewport = vk::Viewport{
        f32(0), f32(_swap_extent.height), f32(_swap_extent.width), -f32(_swap_extent.width), f32(0),
        f32(1)
    };
    // Scissor
    ...
    _pipeline = _device->createGraphicsPipelineUnique(
        nullptr, { vk::PipelineCreateFlags(), _stage_count, _shader_stages.data(),
...
void vulkan_renderer::create_framebuffers()
{
    ...
    for (auto i = 0; i < buffer_count; i++) {
        vector<vk::ImageView> framebuffer_attachments = { _depth_image_view.get(),
                                                          _swapchain_image_views[i] };

        _frame_buffers[i] = _device->createFramebuffer(
            { vk::FramebufferCreateFlags(), _renderpass, u32(framebuffer_attachments.size()),
              framebuffer_attachments.data(), _swap_extent.width, _swap_extent.height, 1 });
...
void vulkan_renderer::create_command_buffers()
{
    ...
    // Command buffers
    _command_buffers =
        _device->allocateCommandBuffers({ _command_pool.get(), vk::CommandBufferLevel::ePrimary,
    ...
    for (u32 i = 0; i < _command_buffers.size(); i++) {
        ...
        vk::RenderPassBeginInfo pass_begin_info(_renderpass, _frame_buffers[i],
...
void vulkan_renderer::draw_frame()
{
    ...
    vk::SubmitInfo submit_info(1, wait_semaphores, wait_stages, 1,
                               &_command_buffers[image_index.value], 1, signal_semaphores);
...
vk::ShaderModule vulkan_renderer::load_shader_module(const std::string& filename)
{
...
void vulkan_renderer::destroy_shader_module(vk::ShaderModule shader) const
{
    _device->destroyShaderModule(shader);
    delete shader;
}

vector<char> vulkan_renderer::read_shader_file(const std::string& filename)
{
...
queue_family_indices vulkan_renderer::detect_queue_families(const vk::PhysicalDevice& device) const
{
...
vk::Extent2D vulkan_renderer::select_swap_extent(const vk::SurfaceCapabilitiesKHR& caps) const
{
