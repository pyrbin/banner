    ...
    const char* pMsg, void* pUserData)
{
    if (flags & VK_DEBUG_REPORT_ERROR_BIT_EXT) {
        std::cerr << "ERROR: [" << pLayerPrefix << "] Code " << msgCode << " : " << pMsg
                  << std::endl;
    } else if (flags & VK_DEBUG_REPORT_WARNING_BIT_EXT) {
        std::cerr << "WARNING: [" << pLayerPrefix << "] Code " << msgCode << " : " << pMsg
                  << std::endl;
    }
...
// Note: window resize may not result in Vulkan telling that the swap chain should be
// recreated, should be handled explicitly!
class TriangleApplication
{
public:
    TriangleApplication() { timeStart = std::chrono::high_resolution_clock::now(); }

    void run()
    {
        // Note: dynamically loading loader may be a better idea to fail gracefully when
        ...
        window = glfwCreateWindow(WIDTH, HEIGHT,
            "The spinning triangle that took 1397 lines of code", nullptr, nullptr);
...
private:
    ...
    void cleanup(bool fullClean)
    {
        ...
        vkFreeCommandBuffers(device, commandPool, (uint32_t)graphicsCommandBuffers.size(),
        ...
        if (fullClean) {
            ...
            // Buffers must be destroyed after no command buffers are referring to them
            // anymore
            ...
            // Note: implicitly destroys images (in fact, we're not allowed to do that
            // explicitly)
    ...
    void findPhysicalDevice()
    {
        ...
        uint32_t supportedVersion[] = { VK_VERSION_MAJOR(deviceProperties.apiVersion),
            VK_VERSION_MINOR(deviceProperties.apiVersion),
            VK_VERSION_PATCH(deviceProperties.apiVersion) };

        std::cout << "physical device supports version " << supportedVersion[0] << "."
                  << supportedVersion[1] << "." << supportedVersion[2] << std::endl;
    }
    ...
    void findQueueFamilies()
    {
        ...
        // Note: is a transfer queue necessary to copy vertices to the gpu or can a
        // graphics queue handle that?
        ...
        if (foundGraphicsQueueFamily) {
            std::cout << "queue family #" << graphicsQueueFamily << " supports graphics"
                      << std::endl;
    ...
    void createDebugCallback()
    {
        if (ENABLE_DEBUGGING) {
            ...
            if (CreateDebugReportCallback(instance, &createInfo, nullptr, &callback)
                != VK_SUCCESS) {
    ...
    void createCommandPool()
    {
        ...
        if (vkCreateCommandPool(device, &poolCreateInfo, nullptr, &commandPool)
            != VK_SUCCESS) {
            std::cerr << "failed to create command queue for graphics queue family"
                      << std::endl;
            exit(1);
        } else {
    ...
    void createVertexBuffer()
    {
        ...
        vkCreateBuffer(
            device, &vertexBufferInfo, nullptr, &stagingBuffers.vertices.buffer);
        ...
        vkBindBufferMemory(
            device, stagingBuffers.vertices.buffer, stagingBuffers.vertices.memory, 0);
        ...
        vkBindBufferMemory(
            device, stagingBuffers.indices.buffer, stagingBuffers.indices.memory, 0);
        ...
        vkCmdCopyBuffer(copyCommandBuffer, stagingBuffers.vertices.buffer, vertexBuffer,
            1, &copyRegion);
        copyRegion.size = indicesSize;
        vkCmdCopyBuffer(copyCommandBuffer, stagingBuffers.indices.buffer, indexBuffer, 1,
            &copyRegion);
    ...
    void updateUniformData()
    {
        ...
        long long millis
            = std::chrono::duration_cast<std::chrono::milliseconds>(timeStart - timeNow)
                  .count();
        ...
        modelMatrix
            = glm::translate(modelMatrix, glm::vec3(0.5f / 3.0f, -0.5f / 3.0f, 0.0f));
        ...
        // Set up projection
        auto projMatrix = glm::perspective(glm::radians(70.f),
    ...
    // Find device memory that is supported by the requirements (typeBits) and meets the
    // desired properties
    ...
    VkExtent2D chooseSwapExtent(const VkSurfaceCapabilitiesKHR& surfaceCapabilities)
    {
        if (surfaceCapabilities.currentExtent.width == -1) {
            ...
            swapChainExtent.width
                = std::min(std::max(WIDTH, surfaceCapabilities.minImageExtent.width),
                    surfaceCapabilities.maxImageExtent.width);
            swapChainExtent.height
                = std::min(std::max(HEIGHT, surfaceCapabilities.minImageExtent.height),
    ...
    void createRenderPass()
    {
        ...
        // Note: this is a description of how the attachments of the render pass will be
        // used in this sub pass e.g. if they will be read in shaders and/or drawn to
    ...
    void createImageViews()
    {
        ...
        for (size_t i = 0; i < swapChainImages.size(); i++) {
            ...
            if (vkCreateImageView(device, &createInfo, nullptr, &swapChainImageViews[i])
                != VK_SUCCESS) {
                std::cerr << "failed to create image view for swap chain image #" << i
                          << std::endl;
                exit(1);
            }
        }
    ...
    void createFramebuffers()
    {
        ...
        // Note: Framebuffer is basically a specific choice of attachments for a render
        // pass That means all attachments must have the same dimensions, interesting
        // restriction
        for (size_t i = 0; i < swapChainImages.size(); i++) {
            ...
            if (vkCreateFramebuffer(
                    device, &createInfo, nullptr, &swapChainFramebuffers[i])
                != VK_SUCCESS) {
                std::cerr << "failed to create framebuffer for swap chain image view #"
                          << i << std::endl;
                exit(1);
            }
        }
    ...
    void createGraphicsPipeline()
    {
        ...
        // Note: depth bias and using polygon modes other than fill require changes to
        // logical device creation (device features)
        ...
        // Note: all attachments must have the same values unless a device feature is
        // enabled
        ...
        // Note: this describes the mapping between memory and shader resources
        // (descriptor sets) This is for uniform buffers and samplers
    ...
    {
        // This describes how many descriptor sets we'll create from this pool for each
        // type
    ...
    void createCommandBuffers()
    {
        ...
        if (vkAllocateCommandBuffers(device, &allocInfo, graphicsCommandBuffers.data())
            != VK_SUCCESS) {
        ...
        for (size_t i = 0; i < swapChainImages.size(); i++) {
            ...
            // If present queue family and graphics queue family are different, then a
            // barrier is necessary The barrier is also needed initially to transition the
            // image to the present layout
            ...
            vkCmdBindDescriptorSets(graphicsCommandBuffers[i],
                VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &descriptorSet, 0,
                nullptr);
            ...
            vkCmdBindVertexBuffers(
                graphicsCommandBuffers[i], 0, 1, &vertexBuffer, &offset);

            vkCmdBindIndexBuffer(
                graphicsCommandBuffers[i], indexBuffer, 0, VK_INDEX_TYPE_UINT32);

            vkCmdDrawIndexed(graphicsCommandBuffers[i], 3, 1, 0, 0, 0);

            vkCmdEndRenderPass(graphicsCommandBuffers[i]);

            // If present and graphics queue families differ, then another barrier is
            // required
            if (presentQueueFamily != graphicsQueueFamily) {
    ...
    void draw()
    {
        ...
        // Unless surface is out of date right now, defer swap chain recreation until end
        // of this frame
        ...
        // Present drawn image
        // Note: semaphore here is not strictly necessary, because commands are processed
        // in submission order within a single queue
