...
int main()
{
    ...
    auto vertexShaderModule = ban::vk_utils::load_shader("shaders/shader.vert.spv", &device.get());
    auto fragmentShaderModule =
        ban::vk_utils::load_shader("shaders/shader.frag.spv", &device.get());
    ...
    auto viewport = vk::Viewport{ 0.0f, 0.0f, static_cast<float>(width), static_cast<float>(height),
        0.0f, 1.0f };
    ...
    auto multisampling =
        vk::PipelineMultisampleStateCreateInfo{ {}, vk::SampleCountFlagBits::e1, false, 1.0 };

    auto colorBlendAttachment =
        vk::PipelineColorBlendAttachmentState{ {}, /*srcCol*/ vk::BlendFactor::eOne,
